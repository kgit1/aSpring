...
public class SpitterDaoJdbcConventional implements SpitterDao {
	...
	public void addSpitter(Spitter spitter) {
		...
		try {
			...
			stmt = conn.prepareStatement(SQL_INSERT_SPITTER); // <co
																// id="co_createStatement"/>

			stmt.setString(1, spitter.getUsername());// <co
														// id="co_bindParameters"/>
	...
	public Spitter getSpitterById(long id) {
		...
		try {
			...
			stmt = conn.prepareStatement(SQL_SELECT_SPITTER);// <co
																// id="co_createStatement"/>
	...
	public void saveSpitter(Spitter spitter) {
		...
		try {
			...
			stmt = conn.prepareStatement(SQL_UPDATE_SPITTER);// <co
																// id="co_createConnection"/>
			...
			// Still not sure what I'm supposed to do here <co
			// id="co_handleExceptions"/>
		} finally {
			try {
				if (stmt != null) {// <co id="co_cleanUp"/>
					stmt.close();
